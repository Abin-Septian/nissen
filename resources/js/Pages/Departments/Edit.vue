<template>
  <div>
    ...
    <trashed-message v-if="department.deleted_at" class="mb-6" @restore="restore">
    ...
    <div class="bg-white rounded shadow overflow-hidden max-w-8xl">
      <form @submit.prevent="submit">
        ...
        <div class="px-8 py-4 bg-gray-100 border-t border-gray-200 flex items-center">
          <button v-if="!department.deleted_at" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy">Delete Department</button>
          <loading-button :loading="sending" class="btn-indigo ml-auto" type="submit">Update Department</loading-button>
        </div>
      </form>
    </div>
  </div>
...
export default {
  ...
  methods: {
    submit() {
      this.sending = true
      this.$inertia.put(this.route('departments.update', this.department.id), this.form)

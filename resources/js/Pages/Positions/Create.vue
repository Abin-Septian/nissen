    ...
    <div class="bg-white rounded shadow overflow-hidden max-w-8xl">
      <form @submit.prevent="submit">
        ...
        <div class="px-8 py-4 bg-gray-100 border-t border-gray-200 flex justify-end items-center">
          <loading-button :loading="sending" class="btn-indigo" type="submit">Create Position</loading-button>
        </div>
      </form>
    </div>
  </div>
...
export default {
  metaInfo: { title: 'Create Position' },
  layout: Layout,
  components: {
    LoadingButton,
  ...
  methods: {
    submit() {
      this.sending = true
      this.$inertia.post(this.route('positions.store'), this.form)
